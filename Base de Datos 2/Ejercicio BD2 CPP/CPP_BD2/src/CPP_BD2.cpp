...
void alta() {
	...
	while(getline(cin, nombre) && nombre.empty()) {
		cout << "El campo de nombre no puede estar vacío!!!. Intente de nuevo: ";
	}
	archivo << nombre << '|';
	cout << "Calle y número: ";
	getline(cin, cYn);
...
void modificacion() {
	...
	while(getline(archivo, linea)) {
		...
		if(temp[0] == s) {
			flag = true;
			cout << "Registro Encontrado!!!\nA continueación modifique los campos que desea cambiar: \n";
			cout << "Introduce el nuevo nombre: ";
			while(getline(cin, nombre) && nombre.empty()) {
				cout << "El campo de nombre no puede estar vacío!!!. Intente de nuevo: ";
			}
			cout << "Nueva Calle y número: ";
			getline(cin, cYn);
			cout << "Nueva Colonia: ";
			getline(cin, col);
		...
		if(temp[0] == s) {
			...
			for(int i = 0 ; i < temp.size() ; i++) {
				if(!i) {
					...
					cout << "Calle y Número: " << temp[1] << '\n';
				} else if(i == 2) {
					cout << "Colonia: " << temp[2] << '\n';
				} else if(i == 3) {
					cout << "Código Postal: " << temp[3] << '\n';
				} else if(i == 4) {
					cout << "Lugar: " << temp[4] << '\n';
				} else if(i == 5) {
					cout << "Teléfono: " << temp[5] << '\n';
				} else if(i == 6) {
					cout << "E-mail: " << temp[6] << '\n';
				} else {
